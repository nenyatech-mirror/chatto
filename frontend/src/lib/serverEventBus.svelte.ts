...
const [getInstanceBusCtx, setInstanceBusCtx] = createContext<ServerEventBus>();
 ...
 * @param serverId - The instance ID (defaults to 'home' for backward compatibility)
 * @returns Cleanup function that stops the subscription
 */
export function initServerEventBus(client: Client, serverId: string = 'home') {
  const cleanup = serverEventBusManager.startBus(serverId, client);
 ...
 * The bus must already be running via serverEventBusManager.startBus().
 *
 * @param serverId - The instance whose bus to provide
 ...
 *
 * @param serverId - The instance to register handlers on
