...
<EnvVar name="CHATTO_CORE_PROJECTION_SNAPSHOTS" toml="core.projection_snapshots" default="false">
  Persist encrypted projection snapshots and replay only the later `EVT` delta for eligible projections at startup. Chatto treats `EVT` as authoritative and automatically replays it when a snapshot is missing, incompatible, corrupt, or unavailable. User profile state is eligible, but password verifiers, external identities, and OAuth consent remain outside snapshots and cold-replay from focused event subjects. While enabled, one elected replica periodically removes unreferenced snapshot objects after a 24-hour grace period. S3 credentials therefore require permission to list the configured bucket prefix in addition to reading, writing, and deleting objects.
</EnvVar>
