...
- Use components from `$lib/ui/form` (`TextInput`, `TextArea`, `Select`, `Checkbox`, `Button`, `FormField`, `FormError`).
- Pass `error` prop to inputs for field-level validation display.
- Wrap custom or raw form controls in `FormField` so labels, descriptions, errors, and vertical spacing match the shared form components. Use the same `id` on `FormField` and the underlying control whenever possible.
