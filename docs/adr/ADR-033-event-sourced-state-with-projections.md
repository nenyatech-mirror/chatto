...
- **Shared consumers for identical projection filters.** Initially each `Projector` ran its own NATS `OrderedConsumer`; after projection startup diagnostics showed duplicate replay cost, `ChattoCore.Run` now groups projectors with identical subject filters onto one ordered consumer and fans each decoded event out to every projection in that group. Each projection still has independent status, lag, failure, and `WaitFor` state, so callers keep the same read-your-writes contract while startup replay and protobuf decode work scale with unique filter sets instead of registered projection count.
...
- Sharing one consumer across merely overlapping, non-identical projection filters. Current grouping is intentionally exact-filter only; broader filter normalization or projection-specific catch-up cursors would be a separate decision.
